...
func NewWithOAuth2Copy(username string, accessToken string, host string, port int) (d *Dialer, err error) {
	...
	var conn *tls.Conn
	//caCert, err := ioutil.ReadFile("Fortinet_CA_SSL.cer")
	//if err != nil {
	//	return nil, err
	//}
	...
	b64 := xoauth2.XOAuth2String(username, accessToken)

	_, err = d.ExecCopy(fmt.Sprintf("AUTHENTICATE XOAUTH2 %s", b64), false, nil)
	if err != nil {
		return nil, err
	}
...
func (d *Dialer) Clone() (d2 *Dialer, err error) {
	...
	fmt.Println("Reconnecting to the email server")
...
func (d *Dialer) Reconnect() (err error) {
	...
	d2, err := d.Clone()
	if err != nil {
		fmt.Println("Imap reconnect error")
...
func (d *Dialer) ExecCopy(command string, buildResponse bool, processLine func(line []byte) error) (response string, err error) {
	var resp strings.Builder
	tag := []byte(fmt.Sprintf("%X", xid.New()))
	...
	for err == nil {
		...
		if len(line) >= taglen+oklen && bytes.Equal(line[:taglen], tag) {
			if !bytes.Equal(line[taglen+1:taglen+oklen], []byte("OK")) {
				return "", fmt.Errorf("imap command failed: %s", line[taglen+oklen+1:])
			}
			break
		}
...
func (d *Dialer) runIdleEvent(data []byte, handler *imap.IdleHandler) error {
	index := 0
	event := ""
	...
	switch event {
	case imap.IdleEventExists:
		if handler.OnExists != nil {
			fetchUIDCommand := fmt.Sprintf("%d FETCH (UID)", index)

			// Execute the fetch command
			uid, _ := d.Exec(fetchUIDCommand, true, 0, func(line []byte) (err error) {
				// Example response: "9 FETCH (UID 64)"
				re := regexp.MustCompile(`(\d+) FETCH \(UID (\d+)\)`)
				matches := re.FindStringSubmatch(string(line))
				if len(matches) == 3 {
					uid, _ := strconv.Atoi(matches[2])
					fmt.Printf("Fetched UID: %d\n", uid)
					// Now you have the UID, you can use it for further operations
				} else {
					return fmt.Errorf("Invalid response format for UID fetch:", string(line))
				}
				return nil
			})
			...
			handler.OnExists(imap.ExistsEvent{MessageIndex: uidINt})
		}
	case imap.IdleEventExpunge:
		if handler.OnExpunge != nil {
			...
			handler.OnExpunge(imap.ExpungeEvent{MessageIndex: uidINt})
		}
	case imap.IdleEventFetch:
		...
		if len(matches) == 4 {
			...
			handler.OnFetch(imap.FetchEvent{MessageIndex: messageIndex, UID: uint32(uid), Flags: flags})
		} else {
...
func (d *Dialer) StartIdle(handler *imap.IdleHandler) error {
	...
	err = retry.Retry(func() (err error) {
		...
		for _, tks := range records {
			...
			for i, t := range tks {
				...
				case "BODY[]":
					...
					if err != nil {
						...
						e.Text = env.Text
						e.HTML = env.HTML
						e.InReplyTo = env.GetHeader("In-Reply-To")
			...
			if success {
				...
				emails[e.UID].InReplyTo = e.InReplyTo
		...
		fmt.Println("Reconnecting to the email server from get emails")
		return err
		return d.Reconnect()
	})
...
func (d *Dialer) GetOverviews(uids ...int) (emails map[int]*Email, err error) {
	...
	err = retry.Retry(func() (err error) {
		...
		fmt.Println("Reconnecting to the email server from get emails overview")
		return err
		return d.Reconnect()
	})
	if err != nil {
